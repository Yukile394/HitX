...
public class HitX implements ClientModInitializer {
    ...
    // ── Trail ────────────────────────────────────────────────
    // 0 = HitColor rengi, 1 = Gökkuşağı (rainbow)
    public static int trailColorMode = 0;

    // ── HUD ──────────────────────────────────────────────────
    public static int  hudX    = 10;
    public static int  hudY    = 10;
    public static boolean hudDragging = false;
    private static int hudDragOX = 0, hudDragOY = 0;

    // ── Particles ────────────────────────────────────────────
    public static float particleColorR = 1.0f;
    public static float particleColorG = 0.15f;
    public static float particleColorB = 0.15f;
    // 0 = tek renk, 1 = gökkuşağı
    public static int   particleColorMode = 0;
    ...
    public static class TrailPoint {
        ...
        public int    age;
        public float  r, g, b;
        public float  hue; // rainbow için
        public TrailPoint(double x, double y, double z, float r, float g, float b, float hue) {
            this.x=x; this.y=y; this.z=z; this.r=r; this.g=g; this.b=b; this.hue=hue;
        }
    }
    ...
    public static final int            JUMP_RING_LIFE = 22;

    // ══════════════════════════════════════════════════════════
    //  Hit Ring veri sınıfı
    // ══════════════════════════════════════════════════════════
    ...
    public static final int           HIT_RING_LIFE = 14;
    ...
    public void onInitializeClient() {
        ...
        ClientTickEvents.END_CLIENT_TICK.register(client -> {
            ...
            if (moving && client.player.age % 2 == 0) {
                float tr, tg, tb;
                if (trailColorMode == 1) {
                    float[] rgb = hsv(rainbowHue, 1.0f, 1.0f);
                    tr=rgb[0]; tg=rgb[1]; tb=rgb[2];
                } else {
                    tr=hitColorR; tg=hitColorG; tb=hitColorB;
                }
    ...
    public class HitXMenu extends Screen {
        ...
        private static final int MW = 270, MH = 172;
        private static final int SP_W = 240, SP_H = 210;
        ...
        public void render(DrawContext ctx, int mx, int my, float delta) {
            ...
            // 5 modül: TriggerBot, Hitboxes, HitColor, Particles, HUD
            String[] mods = {"TriggerBot", "Hitboxes", "HitColor", "Particles", "HUD"};
            int row = oy + 32;
            for (String mod : mods) {
                boolean on  = isOn(mod);
        ...
        private void renderSettingsPanel(DrawContext ctx, MatrixStack ms, int mx, int my) {
            ...
            switch (settingsFor) {
                ...
                case "HitColor" -> {
                    ...
                    py += 4;
                    ctx.drawTextWithShadow(textRenderer, "§7Trail Rengi:", ox+8, py, 0xFF9999BB);
                    py += 12;
                    py = radioButton(ctx, ms, mx, my, ox+8, py, "HitColor Rengi", trailColorMode == 0, 0);
                    py = radioButton(ctx, ms, mx, my, ox+8, py, "Gökkuşağı",      trailColorMode == 1, 1);
                }
                case "Particles" -> {
                    py = slider(ctx, ms, mx, my, ox+8, py, SP_W-16, "§eHız",
                        Math.round(NeonBoxRenderer.particleSpeed * 10), 2, 30,
                        "x" + String.format("%.1f", NeonBoxRenderer.particleSpeed));
                    py = slider(ctx, ms, mx, my, ox+8, py, SP_W-16, "§eSayı",
                        NeonBoxRenderer.particleCount, 2, 20, "adet");
                    py += 2;
                    ctx.drawTextWithShadow(textRenderer, "\u00a77Renk:", ox+8, py, 0xFF9999BB);
                    py += 12;
                    py = radioButton(ctx, ms, mx, my, ox+8, py, "Tek Renk",   particleColorMode == 0, 10);
                    py = radioButton(ctx, ms, mx, my, ox+8, py, "G\u00f6kku\u015fa\u011f\u0131",  particleColorMode == 1, 11);
                    py += 4;
                    if (particleColorMode == 0) {
                        int ppc = packRgb(particleColorR, particleColorG, particleColorB, 1f);
                        fillRound(ms, ox+8, py, SP_W-16, 16, 4f, ppc);
                        outlineRound(ms, ox+8, py, SP_W-16, 16, 4f, 0xFF555566);
                        ctx.drawCenteredTextWithShadow(textRenderer, "Renk \u00d6nizleme", ox+SP_W/2, py+4, 0xFFFFFFFF);
                        py += 22;
                        py = slider(ctx, ms, mx, my, ox+8, py, SP_W-16, "\u00a7cK\u0131rm\u0131z\u0131",
                            Math.round(particleColorR*255), 0, 255, "");
                        py = slider(ctx, ms, mx, my, ox+8, py, SP_W-16, "\u00a7aYe\u015fil",
                            Math.round(particleColorG*255), 0, 255, "");
                        py = slider(ctx, ms, mx, my, ox+8, py, SP_W-16, "\u00a79Mavi",
                            Math.round(particleColorB*255), 0, 255, "");
                    }
                }
                case "HUD" -> {
                    ctx.drawTextWithShadow(textRenderer, "§7HUD Konumu:", ox+8, py, 0xFF9999BB);
                    py += 14;
                    py = slider(ctx, ms, mx, my, ox+8, py, SP_W-16, "X Konum", hudX, 0, 400, "px");
                    py = slider(ctx, ms, mx, my, ox+8, py, SP_W-16, "Y Konum", hudY, 0, 300, "px");
                    ctx.drawTextWithShadow(textRenderer,
                        "§8HUD'ı sürükleyebilirsin (ESC dışında)",
                        ox+8, py, 0xFF444455);
                }
            }
        }
        ...
        // Radio button — id: 0/1 = trail, 10/11 = particles
        private int radioButton(DrawContext ctx, MatrixStack ms, int mx, int my,
                                int x, int y, String label, boolean selected, int id) {
            boolean hov = hovI(mx,my,x,y,14,14);
            fillRound(ms, x, y, 14, 14, 7f, selected ? 0xFF8833CC : 0xFF141424);
            outlineRound(ms, x, y, 14, 14, 7f, selected ? 0xFFBB55EE : 0xFF444455);
            if (selected) fillRound(ms, x+4, y+4, 6, 6, 3f, 0xFFFFFFFF);
            ctx.drawTextWithShadow(textRenderer, label, x+18, y+3,
                hov ? 0xFFCCCCDD : 0xFF888899);
            return y + 20;
        }
        ...
        private void handleSettingsClick(double mx, double my, int ox, int oy) {
            ...
            switch (settingsFor) {
                ...
                case "HitColor" -> {
                    ...
                    // Trail radio
                    py += 35;
                    if (hovD(mx,my, ox+8,py,14,14)) trailColorMode = 0;
                    py += 20;
                    if (hovD(mx,my, ox+8,py,14,14)) trailColorMode = 1;
                }
                case "Particles" -> {
                    // Hız slider
                    int sy = py+11;
                    if (hovD(mx,my, ox+8,sy, SP_W-16,7))
                        NeonBoxRenderer.particleSpeed = MathHelper.clamp(
                            (float)((mx-(ox+8))/(SP_W-16)) * 2.8f + 0.2f, 0.2f, 3.0f);
                    py += 31; sy = py+11;
                    // Sayı slider
                    if (hovD(mx,my, ox+8,sy, SP_W-16,7))
                        NeonBoxRenderer.particleCount = (int)MathHelper.clamp(
                            (float)((mx-(ox+8))/(SP_W-16)) * 18f + 2f, 2, 20);
                    py += 33;
                    // Radio
                    py += 12;
                    if (hovD(mx,my, ox+8,py,14,14)) particleColorMode = 0;
                    py += 20;
                    if (hovD(mx,my, ox+8,py,14,14)) particleColorMode = 1;
                    py += 4;
                    if (particleColorMode == 0) {
                        py += 22;
                        sy = py+11;
                        if (hovD(mx,my, ox+8,sy, SP_W-16,7))
                            particleColorR = MathHelper.clamp((float)((mx-(ox+8))/(SP_W-16)),0,1);
                        py += 31; sy = py+11;
                        if (hovD(mx,my, ox+8,sy, SP_W-16,7))
                            particleColorG = MathHelper.clamp((float)((mx-(ox+8))/(SP_W-16)),0,1);
                        py += 31; sy = py+11;
                        if (hovD(mx,my, ox+8,sy, SP_W-16,7))
                            particleColorB = MathHelper.clamp((float)((mx-(ox+8))/(SP_W-16)),0,1);
                    }
                }
                case "HUD" -> {
                    py += 14;
                    int sy = py+11;
                    if (hovD(mx,my, ox+8,sy, SP_W-16,7))
                        hudX = (int)MathHelper.clamp((mx-(ox+8))/(SP_W-16)*400, 0, 400);
                    py += 31; sy = py+11;
                    if (hovD(mx,my, ox+8,sy, SP_W-16,7))
                        hudY = (int)MathHelper.clamp((mx-(ox+8))/(SP_W-16)*300, 0, 300);
                }
            }
        }
        ...
        private boolean isOn(String n) {
            return switch(n) {
                ...
                case "Particles"  -> particlesActive;
                case "HUD"        -> hudActive;
                default -> false;
            };
        }
        private void toggle(String n) {
            switch(n) {
                ...
                case "Particles"  -> particlesActive  = !particlesActive;
                case "HUD"        -> hudActive        = !hudActive;
            }
        }
