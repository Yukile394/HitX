...
public class HitX implements ClientModInitializer {
    ...
    // Ambient yıldız parçacıkları — sarı/altın + mavi/cyan
    // (tick başına spawn edilir)
    ...
    public void onInitializeClient() {
        ...
        // HUD — ESP kutuları + CatLean tarzı HUD
        HudRenderCallback.EVENT.register((ctx, tick) -> {
            MinecraftClient client = MinecraftClient.getInstance();
            if (client.player == null || client.world == null) return;
            if (client.getDebugHud().shouldShowDebugHud()) return;
            // Sol üst köşe HUD
            renderHUD(client, ctx);
            // ESP
            if (espActive) renderESP(client, ctx);
        });
        ...
        ClientTickEvents.END_CLIENT_TICK.register(client -> {
            ...
            handleCombat(client);
            handleAura(client);
            spawnAmbientParticles(client);
        });
    }

    // ══════════════════════════════════════════════════════════
    //  CatLean Tarzı SOL ÜST HUD
    // ══════════════════════════════════════════════════════════
    private void renderHUD(MinecraftClient client, DrawContext ctx) {
        ...
        // Küçük başlık — "HitX" mor
        ctx.drawTextWithShadow(client.textRenderer, "§dHitX", x, y - 10, 0xFFBB66FF);

        // Her aktif modül için satır
        renderHudLine(ctx, client, ms, x, y,      "Key Binds", true);
        y += 12;
        renderHudLine(ctx, client, ms, x, y,      "Offhand",
            client.player.getOffHandStack() != null && !client.player.getOffHandStack().isEmpty());
        y += 12;
        renderHudLine(ctx, client, ms, x, y,      "Aura",      auraActive);
        y += 12;
        if (aimAssistActive) {
            renderHudLine(ctx, client, ms, x, y,  "AimAssist", true);
            y += 12;
        }
        if (espActive) {
            renderHudLine(ctx, client, ms, x, y,  "ESP",       true);
            y += 12;
        }
        if (triggerBotActive) {
            renderHudLine(ctx, client, ms, x, y,  "TriggerBot",true);
            y += 12;
        }
        if (speedActive) {
            renderHudLine(ctx, client, ms, x, y,  "Speed",     true);
        }
    }

    private void renderHudLine(DrawContext ctx, MinecraftClient client,
                                MatrixStack ms, int x, int y, String name, boolean on) {
        // Sol küçük çizgi — aktifse mavi, değilse gri
        int lineCol = on ? 0xFF44DDFF : 0xFF444455;
        // drawRoundedRect(ms, x, y+1, 2, 9, 1f, lineCol); // ince çizgi
        ctx.fill(x, y+1, x+2, y+10, lineCol);
        int textCol = on ? 0xFFCCEEFF : 0xFF888899;
        ctx.drawTextWithShadow(client.textRenderer, name, x + 5, y, textCol);
    }

    // ══════════════════════════════════════════════════════════
    //  AMBIENT PARTİKEL EFEKTİ — CatLean Tarzı Yıldızlar
    //  Sarı/altın + mavi/cyan parçacıklar etrafta uçar
    // ══════════════════════════════════════════════════════════
    private void spawnAmbientParticles(MinecraftClient client) {
        if (client.player == null || client.world == null) return;
        long now = System.currentTimeMillis();
        // Her 50ms'de bir spawn (20 tick/sn)
        ...
        // ── Sarı/Altın yıldız parçacıkları ──
        // CRIT particle = sarı/altın görünüm
        for (int i = 0; i < 3; i++) {
            ...
            double vy = rng.nextDouble() * 0.12 + 0.05;
            double vz = (rng.nextDouble() - 0.5) * 0.15;
            client.world.addParticle(ParticleTypes.CRIT,
                px+ox, py+oy, pz+oz, vx, vy, vz);
        }

        // ── Mavi/Cyan parçacıklar ──
        // ENCHANTED_HIT = mavi parıltı
        for (int i = 0; i < 2; i++) {
            double ox = (rng.nextDouble() - 0.5) * 5.0;
            double oy = rng.nextDouble() * 4.0;
            double oz = (rng.nextDouble() - 0.5) * 5.0;
            double vx = (rng.nextDouble() - 0.5) * 0.08;
            double vy = rng.nextDouble() * 0.1;
            double vz = (rng.nextDouble() - 0.5) * 0.08;
            client.world.addParticle(ParticleTypes.ENCHANTED_HIT,
                px+ox, py+oy, pz+oz, vx, vy, vz);
        }

        // ── Aura aktifse hedef etrafında ek efekt ──
        if (auraActive && auraLocked != null && auraLocked.isAlive()) {
            double tx = auraLocked.getX();
            double ty = auraLocked.getY() + 1.0;
            double tz = auraLocked.getZ();
            // Hedef etrafında spiral CRIT
            for (int i = 0; i < 2; i++) {
                double angle = (now * 0.003 + i * Math.PI);
                double r = 0.6;
                client.world.addParticle(ParticleTypes.CRIT,
                    tx + Math.cos(angle)*r, ty + rng.nextDouble()*0.5, tz + Math.sin(angle)*r,
                    0, 0.05, 0);
            }
            // Hedef üstünde mavi parıltı
            client.world.addParticle(ParticleTypes.ENCHANTED_HIT,
                tx, ty + 1.2, tz,
